          ...
          case Some(user) if !user.registration.activated =>
            ...
            authTokenService.create(user.id, tokenExpiry).map { authToken =>
              ...
              mailerClient.send(Email(
                ...
                bodyText = Some(auth.twirl.auth.views.txt.emails.activateAccount(user, url).body),
                bodyHtml = Some(auth.twirl.auth.views.html.emails.activateAccount(user, url).body)
              ))
              result
            }
