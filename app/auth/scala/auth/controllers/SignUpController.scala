...
class SignUpController @Inject() (
  ...
  def signUp: Action[AnyContent] = silhouette.UnsecuredAction.async { implicit request =>
    SignUpForm.form.bindFromRequest.fold(
      form => {
        ...
        Logger.debug(request.body.asRaw.toString())
        ...
        Logger.debug(request.body.asXml.toString)
  ...
  private def signUpExistingUser(data: SignUpForm.Data, user: User)(
    ...
    mailerClient.send(Email(
      ...
      to = Seq(data.email),
      // TODO :
      bodyText = Some(auth.twirl.auth.views.txt.emails.alreadySignedUp(user, url).body),
      bodyHtml = Some(auth.twirl.auth.views.html.emails.alreadySignedUp(user, url).body)
    ))
  }
   ...
   *
   * @param data      The form data.
   * @param loginInfo The sign_in info.
   * @param request   The request header.
   * @return A future to wait for the computation to complete.
   */
